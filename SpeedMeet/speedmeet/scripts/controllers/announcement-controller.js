    ...
    function (DAUtility, DALayer) {
        function AnnouncementController(oApplication) {
            ...
            this.getMyAnnouncements = function () {
                ...
                oHttpRequest.url += "lists/getbytitle('" + oSpeedMeetList.Title + "')/items?$select=ID,Title,Created,Status,Participants1/Id,Author/ID,Author/Title&$expand=Participants1/Id,Author/ID,Author/Title&$filter=Participants1/Id eq " + oApplication.CurrentUserId + " &$orderby=Created desc";
                oDALayer.SubmitWebMethod(oHttpRequest).done(function (oListItems) {
                    if (oListItems.d.results) {
                        ...
                        arrayAnnoucements.push(olMeetItems.length);
